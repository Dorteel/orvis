...
#---------------------------
    ...
    def process_segmentation(self, img_msg):
        ...
        try:
            # Convert the ROS Image message to OpenCV format
            cv_image = None  # Initialize cv_image to ensure it exists
            if img_msg.encoding == "bgr8":
                # Convert the ROS Image message to OpenCV format for color images
                cv_image = self.bridge.imgmsg_to_cv2(img_msg, "bgr8")
            elif img_msg.encoding == "16UC1":
                ...
                # Log an error and raise an exception for unsupported encoding
                rospy.logerr(f"Unsupported image encoding: {img_msg.encoding}")
                raise ValueError(f"Unsupported image encoding: {img_msg.encoding}")

            # Create the service request and call the service
            request = ImageSegmentationRequest(image=img_msg)
            response = self.annotator_service(request)

            # If logging level is DEBUG, display the segmentation masks
            if self.logging_level == 'DEBUG' and cv_image is not None:
                self.display_segmentation_masks(cv_image, response)

            # Process the response and update the ontology
            self.create_obs_graph(response)
            return response
...
if __name__ == "__main__":
    try:
        # List of annotator services to test
        annotator_services = [
            # '/annotators/DepthEstimation/dpt_hybrid_midas/detect',
            #'/annotators/ImageSegmentation/detr_resnet_50_panoptic/detect',
            #'/annotators/ImageToText/kosmos_2_patch14_224/detect',
            #'/annotators/ObjectDetection/detr_resnet_50/detect',
            #'/annotators/ObjectDetection/yolos_tiny/detect',
            '/annotators/PromptedObjectDetection/owl_v2/detect',
            '/annotators/VideoClassification/timesformer_hr_finetuned_k600/detect'
        ]

        # Initialize the TaskSelector instance
        annotator_client = TaskSelector()

        # Iterate through each service
        for service_name in annotator_services:
            rospy.loginfo(f"Testing service: {service_name} for 1 minute")
            
            # Start timing
            start_time = time.time()
            elapsed_time = 0

            # Call the service repeatedly for 60 seconds
            while elapsed_time < 60:
                annotator_client.call_service(service_name)
                elapsed_time = time.time() - start_time

            rospy.loginfo(f"Finished testing service: {service_name}")

        rospy.loginfo("Finished testing all annotator services.")
